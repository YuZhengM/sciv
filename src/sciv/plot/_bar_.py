...
def bar_correlation(
    ...
    **kwargs: Any
    ...
    if norm_limit is None:
        max_value = max(abs(df[y].values.min()), df[y].values.max())
        norm_limit = (-max_value, max_value)
    ...
    ax.set_ylim((norm_limit[0] - 0.1, norm_limit[1] + 0.1) if y_limit is None else y_limit)
